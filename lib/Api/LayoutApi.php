...
class LayoutApi
{
     ...
     * Retrieve templates
     *
     * @param  string|null $type Type of the templates you want to get. (optional)
    ...
    public function updateCreditNoteTemplateWithHttpInfo($credit_note_id, $model_change_layout = null, string $contentType = self::contentTypes['updateCreditNoteTemplate'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Itsmind\Sevdesk\Model\UpdateInvoiceTemplate200Response' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\Itsmind\Sevdesk\Model\UpdateInvoiceTemplate200Response' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\Itsmind\Sevdesk\Model\UpdateInvoiceTemplate200Response', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\Itsmind\Sevdesk\Model\UpdateInvoiceTemplate200Response',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
