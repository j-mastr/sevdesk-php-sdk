...
/**
    ...
    public function bookVoucherRequest($voucher_id, $book_voucher_request = null, string $contentType = self::contentTypes['bookVoucher'][0])
    {
        ...
        if (isset($book_voucher_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                ...
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($book_voucher_request));
            } else {
    ...
    public function createVoucherByFactoryRequest($save_voucher = null, string $contentType = self::contentTypes['createVoucherByFactory'][0])
    {
        ...
        if (isset($save_voucher)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($save_voucher));
            } else {
    ...
    public function voucherUploadFileRequest($voucher_upload_file_request = null, string $contentType = self::contentTypes['voucherUploadFile'][0])
    {
        ...
        if (isset($voucher_upload_file_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($voucher_upload_file_request));
            } else {
