...
class ModelCheckAccountResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     * Sets bank_server
     *
     * @param string|null $bank_server Bank server of check account, only set if the account is connected to a data provider
     *
     * @return self
     */
    public function setBankServer($bank_server)
    {
        if (is_null($bank_server)) {
            array_push($this->openAPINullablesSetToNull, 'bank_server');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('bank_server', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
