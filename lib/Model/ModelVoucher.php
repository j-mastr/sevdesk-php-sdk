...
class ModelVoucher implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    protected static $openAPITypes = [
        ...
        'enshrined' => '\DateTime',
        'tax_set' => '\Itsmind\Sevdesk\Model\ModelVoucherTaxSet',
    ...
    */
     ...
     * Sets status
     *
     * @param float $status Please have a look in       <a href='#tag/Voucher/Types-and-status-of-vouchers'>status of vouchers</a>      to see what the different status codes mean
     *
     * @return self
     */
     ...
     * Gets tax_rule
     *
     * @return \Itsmind\Sevdesk\Model\ModelVoucherTaxRule
     */
     ...
     * Sets tax_rule
     *
     * @param \Itsmind\Sevdesk\Model\ModelVoucherTaxRule $tax_rule tax_rule
     *
     * @return self
     */
     ...
     * Sets tax_type
     *
     * @param string $tax_type **Use this in sevdesk-Update 1.0 (instead of taxRule).**  Tax type of the voucher. There are four tax types: 1. default - Umsatzsteuer ausweisen 2. eu - Steuerfreie innergemeinschaftliche Lieferung (Europäische Union) 3. noteu - Steuerschuldnerschaft des Leistungsempfängers (außerhalb EU, z. B. Schweiz) 4. custom - Using custom tax set 5. ss - Not subject to VAT according to §19 1 UStG Tax rates are heavily connected to the tax type used.
     *
     * @return self
     */
     ...
     * Sets voucher_type
     *
     * @param string $voucher_type Type of the voucher. For more information on the different types, check       <a href='#tag/Voucher/Types-and-status-of-vouchers'>this</a>
     *
     * @return self
     */
    ...
    public function setRecurringInterval($recurring_interval)
    {
        ...
        if (!is_null($recurring_interval) && (!preg_match("/^P(?!$)(?:(\\d+)Y)?(?:(\\d+)M)?(?:(\\d+)W)?(?:(\\d+)D)?(?:T(?=\\d)(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?)?$/", ObjectSerializer::toString($recurring_interval)))) {
            throw new \InvalidArgumentException("invalid value for \$recurring_interval when calling ModelVoucher., must conform to the pattern /^P(?!$)(?:(\\d+)Y)?(?:(\\d+)M)?(?:(\\d+)W)?(?:(\\d+)D)?(?:T(?=\\d)(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?)?$/.");
        }
     ...
     * Gets tax_set
     *
     * @return \Itsmind\Sevdesk\Model\ModelVoucherTaxSet|null
     */
     ...
     * Sets tax_set
     *
     * @param \Itsmind\Sevdesk\Model\ModelVoucherTaxSet|null $tax_set tax_set
     *
     * @return self
     */
