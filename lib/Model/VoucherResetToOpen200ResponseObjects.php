...
/**
 * VoucherResetToOpen200ResponseObjects Class Doc Comment
 *
 * @category Class
 * @package  Itsmind\Sevdesk
 * @author   OpenAPI Generator team
 ...
 */
class VoucherResetToOpen200ResponseObjects implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'voucherResetToOpen_200_response_objects';
    ...
    */
    ...
    public function setRecurringInterval($recurring_interval)
    {
        ...
        if (!is_null($recurring_interval) && (!preg_match("/^P(?!$)(?:(\\d+)Y)?(?:(\\d+)M)?(?:(\\d+)W)?(?:(\\d+)D)?(?:T(?=\\d)(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?)?$/", ObjectSerializer::toString($recurring_interval)))) {
            throw new \InvalidArgumentException("invalid value for \$recurring_interval when calling VoucherResetToOpen200ResponseObjects., must conform to the pattern /^P(?!$)(?:(\\d+)Y)?(?:(\\d+)M)?(?:(\\d+)W)?(?:(\\d+)D)?(?:T(?=\\d)(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?)?$/.");
        }
